...
const name = "enrollment_requests";
...
if (!existing) {
  ...
  if (apply) await pb.collections.create({ name, type: definition.type, fields: definition.fields.map((field) => materializeField(field, byName)), indexes: definition.indexes, ...definition.rules }, { requestKey: null });
} else {
  ...
  if (apply && (fieldsChanged || indexesChanged || rulesChanged)) await pb.collections.update(existing.id, { fields, indexes, ...definition.rules }, { requestKey: null });
}
