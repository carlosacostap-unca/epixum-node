...
export default async function LegacySprintsPage({ searchParams }: { searchParams: Promise<LegacySearchParams> }) {
  const [cohort, query] = await Promise.all([getLegacyCohort(), searchParams]);
  redirect(appendSearchParams(cohortPath(cohort.id, "/sprints"), query));
}
