...
export default async function LegacyLongitudinalDashboardPage({ searchParams }: { searchParams: Promise<LegacySearchParams> }) {
  const [cohort, query] = await Promise.all([getLegacyCohort(), searchParams]);
  redirect(appendSearchParams(cohortPath(cohort.id, "/dashboard"), query));
}
