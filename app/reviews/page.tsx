...
export default async function LegacyReviewsPage({ searchParams }: { searchParams: Promise<LegacySearchParams> }) {
  const [cohort, query] = await Promise.all([getLegacyCohort(), searchParams]);
  redirect(appendSearchParams(cohortPath(cohort.id, "/reviews"), query));
}
