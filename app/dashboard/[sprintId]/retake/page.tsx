...
export default async function RetakeSurveysPage({ params }: { params: Promise<{ sprintId: string }> }) {
    ...
    return (
        <div className="container mx-auto p-8 min-h-screen">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-zinc-900 dark:text-white">Solicitan Recursar</h1>
            ...
            <div className="grid grid-cols-1 gap-6">
                {surveys.length === 0 ? (
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-8 text-center text-zinc-500">
                        No hay estudiantes que hayan solicitado recursar.
                    </div>
                ) : (
                    surveys.map((survey) => {
